...
import { useUpdateProductMutation, useGetProductByIdQuery } from "@/store/api/productsApi";
...
const EditProduct = ({ params }: EditProductProps) => {
  ...
  const { data: productData, isLoading: loadingProduct } = useGetProductByIdQuery(parseInt(id));
  ...
  useEffect(() => {
    if (productData?.data) {
      ...
      // Get category_id and subcategory_id, handling both direct ID and nested object cases
      const categoryId = product.category_id || (product.category?.id) || 0;
      const subcategoryId = product.subcategory_id || (product.subcategory?.id) || 0;
      ...
      if (product.variants && product.variants.length > 0) {
        const formVariants: ProductVariant[] = product.variants.map((variant, index) => ({
          id: variant.id?.toString() || (index + 1).toString(),
          image: null,
          imagePreview: typeof variant.image === 'string' ? variant.image : null,
          size: variant.size || "",
          sku: variant.sku || "",
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    // Prevent form submission on Enter key unless on step 2
    if (e.key === 'Enter' && currentStep !== 2) {
      e.preventDefault();
      e.stopPropagation();
      console.log('Enter key prevented - not on step 2, current step:', currentStep);
      return false;
    }
  };
  ...
  const submitProduct = async () => {
    console.log('submitProduct called - Step:', currentStep);
    ...
    formDataToSend.append('name_ar', formData.name_ar.trim());
    
    // Only include category_id and subcategory_id if they are selected (not 0)
    if (formData.category_id && formData.category_id !== 0) {
    ...
    formDataToSend.append('description_ar', formData.description_ar.trim());
    formDataToSend.append('sku', formData.sku.trim());
    formDataToSend.append('is_active', formData.is_active ? '1' : '0');

    // Add variants
    variants.forEach((variant, index) => {
      // Include variant ID if it exists (for existing variants from database)
      // Database IDs are typically small numbers, while Date.now() generates large timestamps
      const variantIdNum = parseInt(variant.id);
      if (variant.id && !isNaN(variantIdNum) && variantIdNum < 1000000000) {
        formDataToSend.append(`variants[${index}][id]`, variant.id);
      }
      
      formDataToSend.append(`variants[${index}][size]`, variant.size.trim());
      formDataToSend.append(`variants[${index}][sku]`, variant.sku.trim());
      formDataToSend.append(`variants[${index}][short_item]`, variant.short_item.trim());
      formDataToSend.append(`variants[${index}][quantity]`, variant.quantity.toString());
      formDataToSend.append(`variants[${index}][price]`, variant.price.toString());
      formDataToSend.append(`variants[${index}][is_active]`, variant.is_active ? '1' : '0');
